<?php
include("fkdb.php");
$query = "SELECT COUNT(*) AS Assign_status FROM assign WHERE Assign_status='Ready'";
$query_result = mysqli_query($conn, $query);
while ($row = mysqli_fetch_assoc($query_result)) {
    $output1 = $row['Assign_status'];
}
?>

<?php
$query = "SELECT COUNT(*) AS Assign_status FROM assign WHERE Assign_status='Assigned'";
$query_result = mysqli_query($conn, $query);
while ($row = mysqli_fetch_assoc($query_result)) {
    $output2 = $row['Assign_status'];
}
?>

<?php
$unassigned = $output1 - $output2;
?>
    ...
    <script type="text/javascript">
        ...
        function drawChart() {
            ...
            var data = google.visualization.arrayToDataTable([
                ['Status ', 'Total'],
                ['Ready', <?php echo $output1 ?>],
                ['Assigned', <?php echo $output2 ?>],
                ['Unassigned', <?php echo $unassigned?>],
            ]);

            var options = {
                title: 'Total Number of Technician '
            };
